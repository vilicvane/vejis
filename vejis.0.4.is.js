/*
    VEJIS 0.4 Intellisense file for Visual Studio 2012
    by VILIC VANE
    www.vilic.info
*/
    ...
    function () {
        ...
        function _(params_Type, fn) {
            ...
            method._ = function (params_Type, fn) {
                ...
                overloads.addOverload(fn, list, true, that);

                if (paramsIndex >= 0) {
                    Types = Types.concat();
                    Types[paramsIndex] = list_(Types[paramsIndex].Type);
                    list = new TypeList(Types);
                    overloads.addOverload(fn, list, false, that);
                }
            };
        ...
        function Overloads() {
            ...
            this.addOverload = function (fn, nList, force, o) {
                ...
                /* IS PART START */
                var Types = nList.Types;
                var pIndex = nList.paramsIndex;
                var PType = nList.ParamsType;

                var values = [];

                for (var i = 0; i < Types.length; i++) {
                    if (i == pIndex)
                        values.push([getInstanceOf(PType)]);
                    else
                        values.push(getInstanceOf(Types[i]));
                }
        ...
        var class_ = _(Function, function (body) {
            ...
            var C = function () {
                ...
                this._ = function (params_Type, fn) {
                    if (!constructor)
                        constructor = _.apply(that, arguments);
                    else
                        constructor._.apply(that, arguments);
                };
